...
class BTD6World(World):
    ...
    def create_items(self) -> None:
        ...
        filler_items = len(self.multiworld.get_unfilled_locations(self.player))-len(self.multiworld.itempool)
    ...
    def create_regions(self) -> None:
        ...
        for i in range(len(all_maps_copy)):
            ...
            region.add_locations({f"{name}-Unlock": f"{name}-Unlock"})
        ...
        for i in range(self.options.max_level.value - 1):
            ...
            xp_region.add_locations(
                {name: self.bloonsMapData.locations[name]}, BTD6Level
            )
        ...
        for i in range(len(Shared.knowledgeIDs)):
            ...
            region.add_locations(
                {Shared.knowledgeIDs[i]: Shared.knowledgeIDs[i]}, BTD6Knowledge
            )
